...
class GaussianMlp(Model):
    def __init__(self, ob_dim, ac_dim):
        super(GaussianMlp, self).__init__(name='GaussianMlp')
        # Here we'll construct a bunch of expressions, which will be
        # used in two places:
        ...
        wd_dict = {}
        h1 = tf.nn.tanh(
            self.dens(ob_no, 64, "h1", weight_init=self.normc_initializer(1.0),
        ...
        mean_na = self.dens(h2, ac_dim, "mean",
                            weight_init=self.normc_initializer(0.1),
                            bias_init=0.0, weight_loss_dict=wd_dict) # Mean control output
        self.wd_dict = wd_dict
        self.logstd_1a = logstd_1a = tf.get_variable(
            "logstd", [ac_dim], tf.float32, tf.zeros_initializer()
        )  # Variance on outputs
        ...
        sampled_ac_na = tf.random_normal(
            tf.shape(ac_dist[:, ac_dim:])) * ac_dist[:, ac_dim:] \
            + ac_dist[:, :ac_dim] # This is the sampled action we'll perform.
        logprobsampled_n = - tf.reduce_sum(
            tf.log(ac_dist[:, ac_dim:]), axis=1) - 0.5 * tf.log(2.0 * np.pi) \
            * ac_dim - 0.5 * tf.reduce_sum(tf.square(ac_dist[:, :ac_dim] \
                                                     - sampled_ac_na) \
        ...
        kl = tf.reduce_mean(self.kl_div(oldac_dist, ac_dist, ac_dim))
