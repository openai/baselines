...
from optimizers.kfac_util import gmatmul, detectMinVal, factorReshape
...
class KfacOptimizer():
    ...
    def getFactors(self, g, varlist):
            ...
            if 'AddN' in bpropOp_name:
                ...
                if fpropOp.op_def.name in KFAC_OPS:
                    ...
                    if len(bInputsList) > 0:
                        ...
                        bTensorShape = fpropOp.outputs[0].get_shape()
                        if len(bTensor.get_shape()) > 0 and bTensor.get_shape()[0].value is None:
                            bTensor.set_shape(bTensorShape)
                        bTensors.append(bTensor)
        ...
        ########
        # check associated weights and bias for homogeneous coordinate
        # representation and check redundent factors TO-DO: there may
        # be a bug to detect associate bias and weights for forking
        # layer, e.g. in inception models.
                    ...
                    if len(factorTensors[item]['bpropFactors']) > 0:
                        if (
                            ...
                            factorTensors[param]['bpropFactors'] = \
                                factorTensors[item]['bpropFactors']
                ...
                if len(factorTensors[param][key]) > 0:
                    if (key + '_concat') not in factorTensors[param]:
                        ...
                        with tf.name_scope(name_scope):
                            factorTensors[param][key + '_concat'] = \
                                tf.concat(factorTensors[param][key], 0)
                else:
                    factorTensors[param][key + '_concat'] = None
                for j, param2 in enumerate(varlist[(i + 1):]):
                    if (len(factorTensors[param][key]) > 0) \
                       and (set(factorTensors[param2][key]) == set(factorTensors[param][key])):
                    ...
                    if fpropFactor is not None:
                        if fpropFactor not in tmpStatsCache:
                            if opType == 'Conv2D':
                                ...
                                if Oh == 1 and Ow == 1 and self._channel_fac:
                                    ...
                                    slot_fpropFactor_stats2 = tf.Variable(
                                        tf.diag(tf.ones([fpropFactor2_size])) * self._diag_init_coeff,
                            ...
                            slot_fpropFactor_stats = tf.Variable(
                                tf.diag(tf.ones([fpropFactor_size])) * self._diag_init_coeff,
                                name='KFAC_STATS/' + fpropFactor.op.name,
                                trainable=False
                            )
                            self.stats[var]['fprop_concat_stats'].append(
                        ...
                        if not((not self._blockdiag_bias) and self.stats[var]['assnWeights']):
                            if bpropFactor not in tmpStatsCache:
                                slot_bpropFactor_stats = tf.Variable(
                                    tf.diag(tf.ones([bpropFactor.get_shape()[-1]])) * self._diag_init_coeff,
    ...
    def computeStatsEigen(self):
        """ compute the eigen decomp using copied var stats to avoid
            concurrent read/write from other queue """
                ...
                if self._factored_damping:
                    ...
                    for e, e_tnorm in zip(eigVals, eigVals_tnorm_avg):
                        ...
                        if len(eigVals) == 1:
                            ...
                            adjustment = tf.pow(
                                tf.pow(e_tnorm, num_factors - 1.) / eig_tnorm_negList_prod,
                                1. / num_factors
                            )
    ...
    def apply_gradients_kfac(self, grads):
        ...
        if self._async:
            ...
            enqueue_op = tf.cond(
                tf.logical_and(
                    tf.equal(tf.mod(self.stats_step, self._kfac_update), tf.convert_to_tensor(0)),
                    tf.greater_equal(self.stats_step, self._stats_accum_iter)
                ),
            ...
            with tf.control_dependencies(dependency_list):
                ...
                if not self._async:
                    ...
                    updateFactorOps = tf.cond(
                        ...
                        lambda: tf.group(*self.applyStatsEigen(self.computeStatsEigen())), no_op_wrapper
                    )
                else:
