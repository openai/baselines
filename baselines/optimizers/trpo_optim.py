...
def gradients(loss, var_list):
    ...
    return [
        g if g is not None else tf.zeros(v.shape)
...
def flatgrad(loss, var_list):
    ...
    return tf.concat([
        tf.reshape(grad, [-1]) for (v, grad) in zip(var_list, grads)
        if grad is not None
    ], 0)
...
def set_from_flat(var_list, flat_theta):
    ...
    for (shape, size, v) in zip(shapes, sizes, var_list):
        assigns.append(
            v.assign(tf.reshape(flat_theta[start:start + size], shape)))
        start += size
...
class TrustRegionOptimization(object):
    ...
    def setup(self, var_list, raw_loss, self_divergence, divergence=None):
        ...
        self.grad_vector_product = sum(
            tf.reduce_sum(g * t)
            for (g, t) in zip(self.divergence_gradient, tangents))
        self.fisher_vector_product = flatgrad(self.grad_vector_product,
                                              var_list)
    ...
    def optimize(self, sess, feed_dict):
        ...
        step_dir = conjugate_gradient(calc_fisher_vector_product,
                                      -loss_flat_grad)
