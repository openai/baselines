...
======= act ========
    ...
    Returns
    -------
    Tensor of dtype tf.int64 and shape (BATCH_SIZE,) with an action to
    be performed for every element of the batch.
...
======= act (in case of parameter noise) ========
    ...
    update_param_noise_threshold_ph: float
        the desired threshold for the difference between non-perturbed
        and perturbed policy
    update_param_noise_scale_ph: bool
        whether or not to update the scale of the noise for the next
        time it is re-perturbed
...
======= train =======
    ...
    weight: np.array
        ...
        multiplied by the importance weight) dtype must be float32 and
        shape must be (batch_size,)

    Returns
    -------
    td_error: np.array
        a list of differences between Q(s,a) and the target in
        Bellman's equation.  dtype is float32 and shape is
        (batch_size,)
...
class DeepDQN(Agent):
    ...
    def scope_vars(self, scope, trainable_only=False):
        ...
        trainable_only: bool
            whether or not to return only the variables that were marked
            as trainable.
        Returns
        -------
        vars: [tf.Variable]
    ...
    def default_param_noise_filter(self, var):
        ...
        # The remaining layers are likely conv or layer norm layers, which
        # we do not wish to perturb (in the former case because they only
        # extract features, in the latter case because we use them for
        # normalization purposes). If you change your network, you will
        # likely want to re-consider which layers to perturb and which to
        # keep untouched.
        return False
    ...
    def build_act(self,
        ...
        make_obs_ph: str -> tf.placeholder or TfInput
            a function that take a name and creates a placeholder of input
            with that name
        q_func: (tf.Variable, int, str, bool) -> tf.Variable
            ...
            and returns a tensor of shape (batch_size, num_actions) with
            values of every action.
        num_actions: int
            number of actions.
        scope: str or VariableScope
            optional scope for variable_scope.
        reuse: bool or None
            whether or not the variables should be reused. To be able to
            reuse the scope must be given.
    ...
    `       See the top of the file for details.
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            q_values = q_func(
                observations_ph.get(), num_actions, scope="q_func")
            ...
            update_eps_expr = eps.assign(
                tf.cond(update_eps_ph >= 0, lambda: update_eps_ph,
                        lambda: eps))
            _act = self.function(
    ...
    def build_act_with_param_noise(self,
                                   make_obs_ph,
                                   q_func,
                                   num_actions,
                                   scope="deepq",
        ...
        """
        Creates the act function with support for parameter space noise
        ...
        param_noise_filter_func: tf.Variable -> bool
            function that decides whether or not a variable should be
            perturbed. Only applicable if param_noise is True. If set to
            None, default_param_noise_filter is used by default.
    ...
    `       See the top of the file for details.
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            # We have to wrap this code into a function due to the way
            # tf.cond() works. See
            # https://stackoverflow.com/questions/37063952/confused-by-the-behavior-of-tf-cond
            # for a more detailed discussion.
            def perturb_vars(original_scope, perturbed_scope):
                all_vars = self.scope_vars(
                    self.absolute_scope_name(original_scope))
                all_perturbed_vars = self.scope_vars(
                    self.absolute_scope_name(perturbed_scope))
            ...
            # Set up functionality to re-compute `param_noise_scale`. This
            # perturbs yet another copy of the network and measures the
            # effect of that perturbation in action space. If the
            # perturbation is too big, reduce scale of perturbation,
            # otherwise increase.
            ...
            updates = [
                update_eps_expr,
                tf.cond(reset_ph, lambda: perturb_vars(
    ...
    def build_train(self,
        ...
        ----------
        make_obs_ph: str -> tf.placeholder or TfInput
            a function that takes a name and creates a placeholder of
    ...
    `       See the top of the file for details.
        ...
        """
        if param_noise:
            act_f = self.build_act_with_param_noise(
                make_obs_ph,
                q_func,
                num_actions,
                scope=scope,
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            q_tp1 = q_func(
                obs_tp1_input.get(), num_actions, scope="target_q_func")
            ...
            update_target = self.function([], [], updates=[update_target_expr])

            q_values = self.function([obs_t_input], q_t)
...
def mlp(hiddens=[], layer_norm=False):
    ...
    """
    return lambda *args, **kwargs: _mlp(
        hiddens, layer_norm=layer_norm, *args, **kwargs
    )
...
def cnn_to_mlp(convs, hiddens, dueling=False, layer_norm=False):
    ...
    return lambda *args, **kwargs: _cnn_to_mlp(
        convs, hiddens, dueling, layer_norm=layer_norm, *args, **kwargs
    )
