...
class CategoricalPdType(PdType, Model):
...
class DiagGaussianPdType(PdType, Model):
    ...
    def pdfromlatent(self, latent_vector, init_scale=1.0, init_bias=0.0):
        ...
        logstd = tf.get_variable(name='logstd', shape=[1, self.size],
...
class MultiCategoricalPd(Pd):
    ...
    def mode(self):
        return tf.cast(
            tf.stack([p.mode() for p in self.categoricals], axis=-1),
            tf.int32
        )

    def neglogp(self, x):
        return tf.add_n(
            [p.neglogp(px) for p, px in zip(self.categoricals,
                                            tf.unstack(x, axis=-1))]
        )
